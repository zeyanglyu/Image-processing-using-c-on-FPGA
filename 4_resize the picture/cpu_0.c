...
for( i = 0; i < sizeY; i++)
   ...
   grayscale_img[i + 3]= (unsigned char)(0.3125 * orig[3 * i + 3 ] + 0.5625 * orig[3 * i + 4] + 0.125  * orig[3 * i + 5]);
	}
printf ("gray:\n");
for( i = 0; i < sizeY; i++)
	{
...
printf ("resize:\n");
for( i = 0; i < resize_img[1]; i++)
	{
	for( j=0; j< resize_img[0];j++)
		{
		printf("%d ",resize_img[j+(i*sizeX/2)+3]);
		}
	printf("\n");
	}
...
{
PERF_BEGIN(PERFORMANCE_COUNTER_0_BASE, asciiSDF3);
...
for( i = 0; i < sizeY; i++)
	...
	PERF_END(PERFORMANCE_COUNTER_0_BASE, asciiSDF3);
}
